...
object LinearRegression1 {
  ...
  // page speed vs amt spent by the user. 
  ...
  val predictions = model.predict(labeled_data_testing.map(_.features));
  
  val results = predictions.zip(labeled_data_testing.map(_.label));
