object LearningScala4 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(285); 
 ...
 val picardsShip = "Picard" -> "Enterprise-D";System.out.println("""picardsShip  : (String, String) = """ + $show(picardsShip ));$skip(35); 
 println("Key: " + picardsShip._1);$skip(37); 
 println("Value: " + picardsShip._2);$skip(89); 
 ...
 for (ship <- shipList) {println(ship)};$skip(174); 
 
 // Let's apply a function literal to a list! map() can be used to apply any function to every item in a collection.
val backwardShips = shipList.map(ship => ship.reverse);System.out.println("""backwardShips  : List[String] = """ + $show(backwardShips ));$skip(45); 
...
val iHateThrees = numberList.filter(_ != 3);System.out.println("""iHateThrees  : List[Int] = """ + $show(iHateThrees ));$skip(47); 

val iHateTwo = numberList.filter(x => x != 2);System.out.println("""iHateTwo  : List[Int] = """ + $show(iHateTwo ));$skip(37); 
for (each <- iHateTwo) println(each);$skip(228); 
...
println(shipMap("Janeway"));$skip(61); 

val n = util.Try(shipMap("Enterprise")) getOrElse "Unknown";System.out.println("""n  : String = """ + $show(n ));$skip(11); 
println(n);$skip(66); 
