...
public class AlienDirectory {
    ...
    public String alienOrder(String[] words) {
        ...
        while(!queue.isEmpty()){
            ...
            for(Character next : nextNeighbors.getOrDefault(temp, new HashSet<Character>())){
                if(!inDegree.containsKey(next)) break;
     ...
     public void buildGraph(String[] words) {
         for(int i=0;i<words.length-1;i++){
             ...
             for(int j=0;j<minLength;j++){
                 ...
                 if(first != second){
                     Set<Character> temp = nextNeighbors.getOrDefault(first, new HashSet<Character>());
