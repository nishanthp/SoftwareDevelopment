...
public class AllPathsFromSourceToTarget {
    ...
    public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
        ...
        for(int i=0;i<graph.length;i++) {
            List<Integer> temp = allPaths.getOrDefault(i, new LinkedList<Integer>());
    ...
    public void dfs(HashMap<Integer, List<Integer>> allPaths, int key, List<Integer> currNeighnors) {
        ...
        if(!currNeighnors.contains(key)) currNeighnors.add(key); // add source node.
        for (Integer curKey : allPaths.getOrDefault(key, new LinkedList<Integer>())){
